...
class Diagnosis_model extends My_Model
{
  ...
  function diagnosis_bayi($data)
  {
    ...
    $simpan = [
      'bayi_id' => $balita['id'], 'usia_ibu' => $usia_melahirkan, 'berat_balita' => $berat_lahir, 'jarak_kehamilan' => $jarak_kehamilan, 'tinggi_balita' => $tinggi_badan, 'z_score' => $z_score['z_score'], 'created_by' => $data['created_by'], 'kesimpulan' => $z_score['stunting'], 'keterangan' => json_encode($this->get_kesimpulan($z_score['stunting']))
    ];
  ...
  function z_score($data)
  {
    ...
    if ($standar_deviasi->num_rows() < 1) {
      return $this->return_failed('Belum ada deviasi yang terdaftar di usia ini.', []);
    }
